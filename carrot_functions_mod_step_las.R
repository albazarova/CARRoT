...
cross_val<-function(vari,outi,c,rule,part,l,we,vari_col,preds,preds_si,mode,cmode,predm,cutoff,objfun,minx=NULL,maxx=NULL,nr=NULL,maxw=NULL,st=NULL,lambdas,preds_tr){
  ...
  las<-glmnet(as.matrix(set_num_las$tr),as.vector(outc),nlambda=100,alpha=1,family=fam,lambda=lambdas)
  ...
  if (mode=='linear'){
    ...
    #full.model<-lm(outi[ra]~.,data=data.frame(vari[ra,]))
    
    full.model<-lm(outc~.,data=data.frame(set_num_las$tr))
    
    step.model<-stepAIC(full.model, direction = "forward", 
                        trace = FALSE)
    
    #pred_st <- predict(step.model, vari[setdiff(1:l,ra),])
    dat<-data.frame(set_num_las$test)
    names(dat)<-names(data.frame(set_num_las$tr))
      
    pred_st <- predict(step.model, dat)
    
    pred_st_tr<-predict(step.model,data.frame(set_num_las$tr))
    
    
  } else {
    
    full.model<-multinom(outc~.,data=data.frame(set_num_las$tr),trace=F)
    ...
    if (objfun=='acc'){
      pred_st <- predict(step.model, dat, type = "class")
      
      pred_st_tr <- predict(step.model, data.frame(set_num_las$tr), type = "class")
    } else {
      
      pred_st <- predict(step.model, dat, type = "probs")
      
      pred_st_tr <- predict(step.model, data.frame(set_num_las$tr), type = "probs")
...
#        predsi_tr<-get_predictions(psi_tr,floor((1-1/part)*l),cutoff,cmode,mode)
        ...
        predsi<-AUC(1-predsi,outi[setdiff(1:l,ra)])
        
        predsi_tr<-AUC(1-predsi_tr,outi[ra])
      }
...
if  (is.factor(predsi)==T){
  
  predsi<-as.numeric(as.character(predsi))
  
  predsi_tr<-as.numeric(as.character(predsi_tr))
...
regr_ind<-function(vari,outi,crv,cutoff=NULL,part=10,mode,cmode='det',predm='exact',objfun='acc',parallel=FALSE,cores,minx=NULL,maxx=NULL,nr=NULL,maxw=NULL,st=NULL){
  ...
  result <- foreach(i=1:crv, .combine='comb', .multicombine=TRUE) %fun% {
    ...
    cross_val(vari,outi,c,rule,part,l,we,vari_col,preds,preds_si,mode,cmode,predm,cutoff,objfun,minx,maxx,nr,maxw,st,lambdas,preds_tr);
