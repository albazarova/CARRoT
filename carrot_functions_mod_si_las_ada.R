...
make_numeric1<-function(vari,outcome,ra){
...
sum_weights_sub<-function(a,m,we,st){
...
compute_max_length<-function(vari_col,k,c,we,maxx=NULL,st){ #
...
cross_val<-function(vari,outi,c,rule,part,l,we,vari_col,preds,preds_si,mode,cmode,predm,cutoff,objfun,minx=NULL,maxx=NULL,nr=NULL,maxw=NULL,st=NULL,lambdas,preds_tr,lambdas2){
  ...
  #defining the set of significant predictors as an empty set
  
  signi=c();
  
  #going through all predictors, for each one of them fitting a univariate regression
  
  for (q in 1:vari_col){
    
    #turning non-numeric variables nto numeric ones 
    
    if (is.numeric(vari[ra,q])==FALSE){
      
      sit<-make_numeric(vari[,q],FALSE,ra,mode) 
    } else {
    ...
    #for the linear mode compute p-value for the f-statistic
    
    if (mode=='linear'){
      
      #re=summary(lm(outc~vari[ra,q]))$coefficients[,4]
      
      re=summary(lm(outc~sit))$fstatistic
      
      if (is.null(re)==F){
        pv <- pf(re[1],re[2],re[3],lower.tail=F)
      } else {
    ...
    else{ #for binary and multinomial modes compute p-value based on likelihood ratio tests as used in the function Anova
      
      re=multinom(outc~.,data=data.frame(vari[ra,q]),trace=FALSE)
      
      # z<-re$coefficients/re$standard.errors
      # 
      # re <- (1 - pnorm(abs(z), 0, 1)) * 2
      # 
      
      pv<-Anova(re,type="III")[3]
    }
    
    #variables exhibiting p-values lower than 0.05 are included inot the set of significant predictors
    
    if (pv<0.05) signi=c(signi,q);
  ...
  set_num_las<-make_numeric_sets(as.matrix(c(1:dim(trset)[2])),1,1,vari,ra,l,mode)
  
  #ri<-glmnet(as.matrix(set_num_las$tr),as.vector(outc), family=fam, alpha=0, standardize=TRUE,lambda=lambdas2)
  
  #ri<-cv.glmnet(as.matrix(set_num_las$tr),as.vector(outc), family=fam, alpha=0, standardize=TRUE,type.measure = "mse",nfold=10)
  
  ri<-lsfit(as.matrix(set_num_las$tr),as.vector(outc))
  
  #preds_las_ri<-c()
  #preds_las_ri_tr<-c()
  
  #for (la in 1:length(lambdas2)){
  
  if (mode=="multin"){
    w3<- rowMeans(1/abs(as.matrix(do.call(cbind,coef(ri,s=ri$lambda[la])))[-1,]))
  } else{
  #w3 <- 1/abs(matrix(coef(ri, s=ri$lambda[la])))
    #w3 <- 1/abs(matrix(coef(ri, s=ri$lambda.min)))
    w3 <- 1/abs(matrix(coef(ri)))
  
  w3[w3[,1] == Inf] <- 999999999
  w3<-w3[-1]
  
  }
    las<-glmnet(as.matrix(set_num_las$tr),as.vector(outc),nlambda=100,alpha=1,family=fam,lambda=lambdas,penalty.factor=w3)
    
    
  preds_las<-array(NA,length(las$lambda))
  preds_las_tr<-array(NA,length(las$lambda))
  ...
  if (mode=="linear"){
    ...
    pred_las<-predict(las,newx=as.matrix(set_num_las$test))
    
    pred_las_tr<-predict(las,newx=as.matrix(set_num_las$tr))
    
   # if (mode=="linear"){
      if (sum(outc%%1)==0){
        
        pred_las<-round(pred_las)
        pred_las_tr<-round(pred_las_tr)
      }
      
    #}
    
    diff_las<--sweep(pred_las,1,outi[setdiff(1:l,ra)])
    
    diff_las_tr<--sweep(pred_las_tr,1,outi[ra])
    
    #diff<-outi[setdiff(1:l,ra)]-pred_las;
    
    diffr_las<-(pred_las/outi[setdiff(1:l,ra)])-1;
    
    diffr_las_tr<-(pred_las_tr/outi[ra])-1;
    
    preds_las=colSums(abs(diff_las));
    
    preds_las_tr=colSums(abs(diff_las_tr));
    
    predsr_las=colSums(abs(diffr_las));
    
    predsr_las_tr=colSums(abs(diffr_las_tr));
    ...
    pred_las<-apply(predict(las,newx=as.matrix(set_num_las$test),type="class"),2,as.numeric)
    
    pred_las_tr<-apply(predict(las,newx=as.matrix(set_num_las$tr),type="class"),2,as.numeric)
    
    
    diff_las<--sweep(as.matrix(pred_las),1,outi[setdiff(1:l,ra)])
    
    diff_las_tr<--sweep(as.matrix(pred_las_tr),1,outi[ra])
    ...
    if (predm=='exact')  {
      ...
      diff_las[abs(diff_las)>0]=1;
      diff_las_tr[abs(diff_las_tr)>0]=1;
    } else{
      
      
      diff_las[abs(diff_las)<2]=0;
      diff_las[abs(diff_las)>1]=1;
    }
    
    
    preds_las=l-floor((1-(1/part))*l)-colSums(abs(diff_las));
    preds_las_tr=floor((1-(1/part))*l)-colSums(abs(diff_las_tr));
    #predspos[j,k]=sum(pred);
    
    
  } else{
    
    pp<-predict(las,newx=as.matrix(set_num_las$test),type="response")
    
    pp_tr<-predict(las,newx=as.matrix(set_num_las$tr),type="response")
    
    
    for (lll in 1:dim(pp)[2]) {
      preds_las[lll]=AUC(pp[,lll],outi[setdiff(1:l,ra)])
      preds_las_tr[lll]=AUC(pp_tr[,lll],outi[ra])
  ...
  #preds_las_ri<-c(preds_las_ri,preds_las)
  #preds_las_ri_tr<-c(preds_las_ri_tr,preds_las_tr)
  
  
  #}
  
  #preds_las<-preds_las_ri
  #preds_las_tr<-preds_las_ri_tr
    ...
    if (length(ai)>0){
      ...
      for (k in 1:length(ai)){
        ...
        if (mode=='linear'){
          ...
          pred_tr<-get_predictions_lin(trset1,trset1,outc,floor((1-1/part)*l))
          ...
          preds_tr[j,k]=sum(abs(diff_tr));
          
          predsr[j,k]=sum(abs(diffr));
          
          predsr_tr[j,k]=sum(abs(diffr_tr));
          
          # if (all(a[,ai[k]] %in% signi)){
          #   
          #   pred_si<-get_predictions_lin(trset1,testset1,outc,l-floor((1-1/part)*l))
          #   
          #   diff_si<-outi[setdiff(1:l,ra)]-pred_si;
          #   
          #   diff_sir<-(pred_si/outi[setdiff(1:l,ra)])-1;
          #   
          #   preds_si[j,k]=sum(abs(diff_si));
          #   
          #   preds_sir[j,k]=sum(abs(diff_sir));
          #   
          # }
          ...
          p_tr<-get_probabilities(trset1,trset1,outc,mode);
          ...
          if (objfun=='acc'){
            ...
            preds_tr[j,k]=floor((1-1/part)*l)-sum(abs(diff_tr));
            #predspos[j,k]=sum(pred);
            ...
            preds_tr[j,k]<-AUC(1-p_tr,outi[ra])
          }
        }
  ...
  #if the set of significant predictors is not empty convert the corresponding variables into numeric ones
  
  if (is.null(signi)==FALSE){
    set_num_si<-make_numeric_sets(matrix(signi,ncol=1),1,length(q),vari,ra,l,mode)
    
    testset1si<-set_num_si$test
    
    trset1si<-set_num_si$tr
  }
  
  if (mode=='linear'){#computing predictions based on the set of significant predictors in case of linear regression
    ...
    if (is.null(signi)==FALSE){
      ...
      predsi<-get_predictions_lin(trset1si,testset1si,outc,l-floor((1-1/part)*l))
      
      predsi_tr<-get_predictions_lin(trset1si,trset1si,outc,floor((1-1/part)*l))
    ...
    if (is.null(signi)==FALSE){#computing predictions based on the set of significant predictors in case of logistic or multinomial regression
      
      #computing probabilities on the test set
      
      psi<-get_probabilities(trset1si,testset1si,outc,mode)
      
      psi_tr<-get_probabilities(trset1si,trset1si,outc,mode)
      
      if (objfun=='acc'){
        
        #computing predictions on the test set given cut-off
        
        predsi<-get_predictions(psi,l-floor((1-1/part)*l),cutoff,cmode,mode)
        
        predsi_tr<-get_predictions(psi_tr,floor((1-1/part)*l),cutoff,cmode,mode)
        
        
      }
      else{
        
        #computing AUROC value
        
        predsi<-AUC(1-psi,outi[setdiff(1:l,ra)])
        
        predsi_tr<-AUC(1-psi_tr,outi[ra])
      }
  ...
  if ((mode=='binary')&(objfun=='acc')){
    ...
    predsi=1-sum(abs(outi[setdiff(1:l,ra)]-predsi)/(l-floor((1-1/part)*l)));
    
    predsi_tr=1-sum(abs(outi[ra]-predsi_tr)/(floor((1-1/part)*l)));
    
    #writing the predictive power as the last element of the output of the function cross_val
    
    list("regr" = preds, "nvar"=nvar, "emp" = cpred, "regrsi"=predsi, "regrlas"=preds_las,"regrsi_tr"=predsi_tr,"regrlas_tr"=preds_las_tr,"regr_tr"=preds_tr)
    ...
    if ((mode=='multin')|(objfun=='roc')) {
      ...
      if (mode=='multin')  {
        ...
        predsi=1-length(which(abs(outi[setdiff(1:l,ra)]-array(predsi))>0))/(l-floor((1-1/part)*l));
        
        predsi_tr=1-length(which(abs(outi[ra]-array(predsi_tr))>0))/floor((1-1/part)*l);
        
        uo<-unique(outc);
        
        cpred=1-length(which(outi[setdiff(1:l,ra)]==uo[which.max(tabulate(match(outc,uo)))]))/(l-floor((1-1/part)*l));
        
        #writing the predictive power as the third element of the output of the function cross_val
        
        list("regr" = preds, "nvar"=nvar, "regrsi"=predsi, "emp"=cpred, "regrlas"=preds_las, "regrsi_tr"=predsi_tr,"regrlas_tr"=preds_las_tr,"regr_tr"=preds_tr) 
      } else {
        
        #writing the predictive power as the last element of the output of the function cross_val
        
        list("regr" = preds, "nvar"=nvar, "regrsi"=predsi, "regrlas"=preds_las,"regrsi_tr"=predsi_tr,"regrlas_tr"=preds_las_tr,"regr_tr"=preds_tr)
      }
      ...
      #for linear mode computing the absolute error based on the set of significant predictors
      
      cpredsi=sum(abs(outi[setdiff(1:l,ra)]-predsi)/(l-floor((1-1/part)*l)));
      
      cpredsi_tr=sum(abs(outi[ra]-predsi_tr)/(floor((1-1/part)*l)));
      
      #for linear mode computing the relative error based on the set of significant predictors
      
      cpredsir=(sum(abs((outi[setdiff(1:l,ra)]-predsi)/outi[setdiff(1:l,ra)]))/(l-floor((1-1/part)*l)));
      
      cpredsir_tr=(sum(abs((outi[ra]-predsi_tr)/outi[ra]))/floor((1-1/part)*l));
      
      
      #writing the predictive power as the last two elements of the output of the function cross_val
      
      list("regr" = preds, "nvar"=nvar, "regrr"=predsr, "emp"=cpred,"empr"=cpredr,"regrsi"=cpredsi, "regrsir"=cpredsir, "regrlas"=preds_las,"regrlasr"=predsr_las,"regrsi_tr"=cpredsi_tr,"regrsir"=cpredsir_tr,"regrlas_tr"=preds_las_tr,"regrlasr_tr"=predsr_las_tr,"regrr_tr"=predsr_tr,"regr_tr"=preds_tr)
...
regr_ind<-function(vari,outi,crv,cutoff=NULL,part=10,mode,cmode='det',predm='exact',objfun='acc',parallel=FALSE,cores,minx=NULL,maxx=NULL,nr=NULL,maxw=NULL,st=NULL){
  ...
  le<-compute_max_length(vari_col,floor((1.0/rule)*numr),c,we,maxx+1,st)
  
  
  preds<-array(NA,c(min(vari_col,min(floor((1/rule)*numr),maxx+1)),le));
  
  preds_tr<-array(NA,c(min(vari_col,min(floor((1/rule)*numr),maxx+1)),le));
  
  
  preds_si<-array(NA,c(min(vari_col,min(floor((1/rule)*numr),maxx+1)),le));
  
  preds_si_tr<-array(NA,c(min(vari_col,min(floor((1/rule)*numr),maxx+1)),le));
  ...
  set_num<-make_numeric_sets(as.matrix(c(1:dim(trset)[2])),1,1,vari,ra,l,mode)
  
  lambdas<-glmnet(as.matrix(set_num$tr),as.vector(outc),nlambda=100,alpha=1,family=fam)$lambda
  
  lambdas2<-glmnet(as.matrix(set_num$tr),as.vector(outc),nlambda=100,alpha=0,family=fam,standardize = T)$lambda
  ...
  result <- foreach(i=1:crv, .combine='comb', .multicombine=TRUE) %fun% {
    ...
    cross_val(vari,outi,c,rule,part,l,we,vari_col,preds,preds_si,mode,cmode,predm,cutoff,objfun,minx,maxx,nr,maxw,st,lambdas,preds_tr,lambdas2);
  ...
  if((mode=='multin')|(objfun=='roc')) {
    ...
    preds_las<-result[[4]]
    predsi_tr<-result[[5]]
    preds_las_tr<-result[[6]]
    
    } else {
      preds_las<-result[[5]]
      predsi_tr<-result[[6]]
      preds_las_tr<-result[[7]]
    }
  }
  ...
  if (mode=='binary') {
    ...
    if (mode=='linear'){
      ...
      predsir_tr<-result[[11]]
      
      preds_las_tr<-result[[12]]
      
      predsr_las_tr<-result[[13]]
  ...
  if (objfun=='roc'){
    ...
    predsp_las<-av_out(preds_las,crv,1)
    
    predsp_las_tr<-av_out(preds_las_tr,crv,1)
    ...
    predsp_las<-av_out(as.matrix(preds_las),crv,l-floor((1-(1/part))*l))
    
    
    predsp_tr<-av_out(preds_tr,crv,floor((1-(1/part))*l))
    
    predsp_las_tr<-av_out(as.matrix(preds_las_tr),crv,floor((1-(1/part))*l))
  ...
  if (mode=='linear')  {
    ...
    predspr_las<-av_out(as.matrix(predsr_las),crv,l-floor((1-(1/part))*l))
    
    predspr_tr<-av_out(predsr_tr,crv,floor((1-(1/part))*l))
    
    predspr_las_tr<-av_out(as.matrix(predsr_las_tr),crv,floor((1-(1/part))*l))
    
    # predsp_si<-av_out(preds_si,crv,l-floor((1-(1/part))*l))
    # 
    # predsp_sir<-av_out(preds_sir,crv,l-floor((1-(1/part))*l))
  }
  ...
  if (((mode=='binary')&(objfun=='acc'))|((mode=='multin'))) {
    ...
    t2<-max(predsp_las,na.rm=T)
    t2_ind<-which(predsp_las==t2)[1]
    if (is.infinite(t2)) t2=NA
    
    print(c(t1,predsp_tr[t1_ind],1-sum(cpred)/crv,sum(predsi)/crv,sum(predsi_tr)/crv,t2,predsp_las_tr[t2_ind]));
    
    a1<-c(t1,predsp_tr[t1_ind],1-sum(cpred)/crv,sum(predsi)/crv,sum(predsi_tr)/crv,t2,predsp_las_tr[t2_ind])
    ...
    if (objfun=='roc') {
      ...
      print(c(t1,predsp_tr[t1_ind],sum(predsi[predsi<Inf],na.rm=TRUE)/(crv-length(which(predsi==Inf))),sum(predsi_tr[predsi_tr<Inf],na.rm=TRUE)/(crv-length(which(predsi_tr==Inf))),t2,predsp_las_tr[t2_ind]))
      
      a1<-c(t1,predsp_tr[t1_ind],sum(predsi[predsi<Inf],na.rm=TRUE)/(crv-length(which(predsi==Inf))),sum(predsi_tr[predsi_tr<Inf],na.rm=TRUE)/(crv-length(which(predsi_tr==Inf))),t2,predsp_las_tr[t2_ind])
      ...
      t2<-min(predsp_las,na.rm=T)
      t2_ind<-which(predsp_las==t2)[1]
      if (is.infinite(t2)) t2=NA
      
      t3<-min(predspr[predspr>0],na.rm=TRUE)
      t3_ind<-which(predspr==t3)[1]
      if (is.infinite(t3)) t3=NA
      
      t4<-min(predspr_las,na.rm=T)
      t4_ind<-which(predspr_las==t4)[1]
      if (is.infinite(t4)) t4=NA
      
      #    print(c(min(predsp[predsp>0],na.rm=TRUE),min(predspr[predspr>0],na.rm=TRUE),mean(cpred[cpred<Inf],na.rm=TRUE),mean(cpredr[cpredr<Inf],na.rm=TRUE),mean(predsi[predsi<Inf],na.rm=TRUE),mean(predsir[predsir<Inf],na.rm=TRUE),min(predsp_si[predsp_si>0],na.rm=TRUE),min(predsp_sir[predsp_sir>0],na.rm=TRUE)))
      print(c(t1,predsp_tr[t1_ind],t3,predspr_tr[t3_ind],mean(cpred[cpred<Inf],na.rm=TRUE),mean(cpredr[cpredr<Inf],na.rm=TRUE),mean(predsi[predsi<Inf],na.rm=TRUE),mean(predsi_tr[predsi_tr<Inf],na.rm=TRUE),mean(predsir[predsir<Inf],na.rm=TRUE),mean(predsir_tr[predsir_tr<Inf],na.rm=TRUE),t2,predsp_las_tr[t2_ind],t4,predspr_las_tr[t4_ind]))
      
      a1<-c(t1,predsp_tr[t1_ind],t3,predspr_tr[t3_ind],mean(cpred[cpred<Inf],na.rm=TRUE),mean(cpredr[cpredr<Inf],na.rm=TRUE),mean(predsi[predsi<Inf],na.rm=TRUE),mean(predsi_tr[predsi_tr<Inf],na.rm=TRUE),mean(predsir[predsir<Inf],na.rm=TRUE),mean(predsir_tr[predsir_tr<Inf],na.rm=TRUE),t2,predsp_las_tr[t2_ind],t4,predspr_las_tr[t4_ind])
    }
  ...
  if (mode=='linear') {
    ...
    #   list(get_indices(-predsp,nvar,c,we,st),get_indices(-predspr,nvar,c,we,st),get_indices(-predsp_si,nvar,c,we,st),get_indices(-predsp_sir,nvar,c,we,st))
    
    list(a1,get_indices(-predsp,nvar,c,we,st),get_indices(-predspr,nvar,c,we,st))
    
    
  } else{
    
    list(a1,get_indices(predsp,nvar,c,we,st))
...
regr_whole<-function(vari,outi,crv,cutoff=NULL,part=10,mode,cmode='det',predm='exact',objfun='acc',parallel=FALSE,cores,minx=NULL,maxx=NULL,nr=NULL,maxw=NULL,st=NULL){
  ...
  ind<-regr_ind(vari,outi,crv,cutoff,part,mode,cmode,predm,objfun,parallel,cores,minx,maxx,nr,maxw,st)
